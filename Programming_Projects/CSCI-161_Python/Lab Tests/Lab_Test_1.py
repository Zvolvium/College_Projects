...
avgWeight = sum(peopleWeights)/(len(peopleWeights))
print('\nAverage weight: %1.2f' % avgWeight)

# Output max weight from list.
print('Max weight: %1.2f'% max(peopleWeights))

# Prompt user for a list index & output that weight in lbs & kgs.
index = int(input('\nEnter a list index (1 - 4):\n'))
chosenWeight = peopleWeights[index - 1]
print('Weight in pounds: %1.2f' % chosenWeight)
print('Weight in kilograms: %1.2f' % (chosenWeight/2.2))
