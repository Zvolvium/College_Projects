...
void ReadData (List *head){ // Works, as well as Done :)
  FILE *data; data = fopen("hw7data.txt", "r");
  if (data == NULL){ printf("\nError: File 'hw7data.txt' Failed to Open!!\n"); fclose(data); exit(0); }
...
void ReadCodex (Codex *head){ // Works, as well as Done :)
  FILE *codex; codex = fopen("hw7codex.txt", "r");
  if (codex == NULL){ printf("\nError: File 'hw7codex.txt' Failed to Open!!\n"); fclose(codex); exit(0); }
...
void ReplaceWords (List *lhead, Codex *chead){ // Words, as well as Done :)
  ...
  while (lcurr != NULL){
    ...
    while (ccurr != NULL){
      if (strcmp(lcurr->word, ccurr->word) == 0){ // If words are same replace with correct word
        ...
        strcpy(lcurr->word, ccurr->correction);
      } ccurr = ccurr->next;
    } lcurr = lcurr->next;
  } printf("Word Replacing Complete\n\n");
}

void PrintPoem (List *lhead){ // Works, as well as Done :)
...
int main (void){
  ...
  */
  ...
  ParsePunctuation(listHead); ReplaceWords(listHead, codexHead); PrintPoem(listHead);
