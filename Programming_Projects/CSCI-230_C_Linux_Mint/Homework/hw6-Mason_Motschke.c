...
struct doublyLinkedList { char name[42]; char number[8]; int *last; int *next; };
...
int OpenSpace(struct data directory[], struct doublyLinkedList FAT[], char choice[]){
...
void addFirstNode(struct data directory[], struct doublyLinkedList FAT[], char class[], char name[], char number[]){
	int openIndexDirect = OpenSpace(directory, FAT, "Direct"); int Dcurr = openIndexDirect;
	int openIndexFAT = OpenSpace(directory, FAT, "FAT"); int Fcurr = openIndexFAT;
...
void addNode(struct data directory[], struct doublyLinkedList FAT[], char class[], char name[], char number[]){
	int openIndexFAT = OpenSpace(directory, FAT, "FAT"); int Fcurr = openIndexFAT;
...
int main(void){
	int size = Scan(); struct data directory[10]; struct doublyLinkedList FAT[100];
	...
	for (index = 0; index < size; index++){
		...
		if (strcmp(letter, "a") == 0){ // If letter is 'a' <--------------------------------------------------------------------------
			...
			if (inDirect == 0){ addFirstNode(directory, FAT, class, name, number); } // If class is not in directory
			else if (inDirect == 1){ addNode(directory, FAT, class, name, number); } // If class is in directory      'a' = completed !!
	...
	fclose(fp); printf("File Read\n\n"); PrintAll(directory, FAT); FreeAll(directory, FAT); return 0;
}
